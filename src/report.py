...
def add_to_report(report: mne.Report, subject_num: str):
    ...
    report.add_figure(psd.plot(), title=config.report_titles['psd'], section=config.report_sections['psd'], replace=True)


    #plot csds (computed for epochs_combined[condition]):
    for condition in [list(epochs_combined.event_id.keys()), 'baseline']:    
        csd = read_csd(config.csd_path)

        csd_mean = read_csd(config.csd_mean_path)

        report.add_figure(csd.plot(show=False), title= config.report_titles['csd'], section=config.report_sections.csd, replace=True)

        report.add_figure(csd_mean.plot(show=False), title= config.report_titles['csd_mean'], section=config.report_sections['csd'], replace=True)

        report.add_figure(csd_mean.plot(mode='coh', show=False), title= config.report_titles['coherence'], section=config.report_sections['coherence'], replace=True)



    #plot gfp for evoked:
    report.add_figure(evoked.plot_image(titles=f"Global Field Power for a single subject", show=False),title=config.report_titles['gfp'], 
    section=config.report_sections['gfp'], replace=True)
   ...
   #plot tfr contrast computed per contrast (evoked[condition_1] - evoked[condition_2]):
    for file in glob.glob("*tfr*.h5"):
        ...
        title= config.report_titles['tfr_contrast'], section=config.report_sections['tfr_contrast'], replace=True)
        ...
        for time_range in config.time_frames:
            ...
            title=config.report_titles['tfr_contrast_topoplots'], section=config.report_sections['tfr_contrast_topoplots'], replace=True)
